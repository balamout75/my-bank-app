...
В этой работе я вышел из зоны комфорта — предыдущие проекты были на Gradle и WebFlux,
здесь использованы **Maven** и классический **Spring MVC**. Это позволило глубже понять
традиционный servlet-стек. Дополнительной сложностью стало использование **Spring Boot 4** —
многие библиотеки и подходы потребовали адаптации.

Для организации мониторинга микросервисного приложения в Kubernetes развёрнут единый
namespace `monitoring`. Рассматривалось несколько подходов:

- развернуть инструменты мониторинга непосредственно в боевых namespace
  (`mybank`, `mybank-test`, `mybank-prod`) — решение показалось излишне экстенсивным
  даже для учебного проекта;
- вынести инструменты мониторинга в обособленный namespace — этот вариант и был выбран.

Схему можно усовершенствовать через проверку существования namespace в среде
развёртывания, однако в реальных проектах, где мне доводилось работать, инфраструктура
всегда была подготовлена заранее. Это условие принято и здесь: деплой осуществляется
не на пустом месте, серверные пространства подготовлены предварительно.

Что понравилось, на что обратил внимание. 
- Zipkin. Задача по поддержанию Dependency tree в рабочем состоянии держала в тонусе. Надо чтобы и trace не рвались, и хранение было обеспечено, и шло в ES.
- Prometheus - это про метрики, Kibana - это про логи. В принципе, задачи можно решать одинаковые. Результаты notification сейчас идут в Kibana, но можно было бы направить и в Grafana/
- ES - капризный. Красивого решения по установке всего ELK стека я не нашел, пришлось ставить из образов в DockerHub
- Готовые k8s кластеры очень тяжелые

За что должен извиниться
- Да, много кода. Вынужденное бездействие дает мне возможность достаточно много времени заниматься домашкой. На этот проект ушло не меньше 70 часов.
- Много кода порождает много сопутствующих проблем. Банальный отказ от использования dependencies Spring Cloud (Eureka/LoadBalancer) привел к утрате транзитивных зависимостей 
и это заставило поискать документацию и поразбираться. Одно только переписывание тестов заняло полдня
- По параллельной сборке - с ключами -TC - я попробовал, и еще воспользуюсь этим инструментом. Но тут надо прямо выстраивать граф очередностей, основанный на зависимостях. Компилировать по схеме 2 сервиса + 2 +1.
- По пробрасыванию переменных в keycloak. Интересно получилось, но я сделаю в будущем попроще. Всеж заводить целую виртуальную машину как генератор json .. Ну, в каких то случаях это необходимо, оставлю это решение как вариант. Кажется, можно было шаблонизаторами это решить.  
- Ну и спасибо, всем кто прочтет этот текст. Очень признателен за поддержку.
